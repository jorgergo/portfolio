...
2. Write the Vitest cases for `formatRowNumber` (`src/lib/site-nav.test.ts`) and `formatProfileHandle` (added to `src/lib/cv-format.test.ts`), tagged `AC-8`, satisfies **AC-8**.
3. Add `NavRow` to `/styleguide` (the pinned numbered and keyed examples in both panels, `gap-1` and `gap-4` in its spacing list) and to `design.md` (nine components, the `NavRow` rule and the interactive rows line, the spacing meanings, the prefix widths, `text-pretty`), satisfies **AC-9**.
