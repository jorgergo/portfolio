...
- [x] `grep -o 'href="[^"]*"' dist/index.html` → `#main` (the skip link), `/cv`, `https://github.com/jorgergo`, `https://www.linkedin.com/in/jorgergo/`, `mailto:jorgergo@icloud.com`, plus the shell's stylesheet, favicon, and font preload; `grep -c 'target=' dist/index.html` → 0 → AC-2, AC-3, AC-4
- [x] `grep -oE 'aria-hidden="true">\s*01\s*<' dist/index.html` → the number span is hidden (`compressHTML` pads the text inside each span with a space on either side); `grep -oE 'w-20">\s*github\s*<' dist/index.html` → the key is plain text with no `aria-hidden` on its span → AC-3, AC-4
- [x] `pnpm preview` then `curl -s -o /dev/null -w '%{http_code}\n' http://localhost:8787/cv` → 200 for every href in the Pages nav (one line per href) → AC-10
