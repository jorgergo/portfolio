...
- **AC-11**: The `jorgergo.dev` zone has these settings: Email Address Obfuscation off, Rocket Loader off, Automatic HTTPS Rewrites off, Web Analytics automatic setup off, Zaraz not set up, Bot Fight Mode off, AI bot blocking off, managed `robots.txt` off, the free plan Content Signals Policy left on (with no `robots.txt` in `dist/`, the zone answers `/robots.txt` with Cloudflare's comments only text, which sets no signals and no rules, so every crawler stays allowed), AI Labyrinth off, Browser Integrity Check off (it can challenge the smoke check's curl), Cloudflare Fonts off and Automatic Signed Exchanges off (both can change what a page serves), Always Use HTTPS on, Minimum TLS Version 1.2, the zone HSTS setting off (the header comes from `_headers`), DNSSEC on, and domain auto renew on.
