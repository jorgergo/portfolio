...
- **A name part over one card line fails the schema (you chose this).** Parts split at whitespace (not at a no break space) and right after a hyphen, the break points Satori uses (measured: `Wolfeschlegel-Steinhausenberg` breaks after the hyphen and stays inside the padding). Runner up: split at whitespace only, which would reject a hyphenated name the card draws fine. Breaking the word on the card or shrinking the name were declined: one splits a name mid word, the other changes the balanced size.
...
| `missingGlyphs(text)` (`share-card.ts`) | pure | `string` | `readonly string[]`: each character outside `CARD_GLYPHS` or inside `FONT_GAPS`, once, in order; empty when all are drawable | n/a | none |
...
| Validate `cv.json` | the characters a card may hold | `CARD_GLYPHS`: `parseUnicodeRange(unicode[FONT_SUBSET])`, from `@fontsource/ibm-plex-mono/unicode.json`, less `FONT_GAPS` (read from the woff cmap) |
...
- The subset has one name (`FONT_SUBSET`): the font files the cards load and the characters the schema allows always come from the same `unicode.json` entry, less the `FONT_GAPS` the files lack; the Satori render test fails if the two drift.
...
13. [x] Found by `/test` on 2026-09-25: the `unicode.json` range lists 163 code points the woff files lack, and `\s` split name parts at no break spaces the card cannot break. Add `FONT_GAPS` to `share-card.ts` and the Satori render test for every accepted character; split name parts only at breaking whitespace; add the `U+2010` and `U+00A0` cases, satisfies **AC-15**, **AC-16**.
...
- A name part over 24 characters fails the build, and the city is capped at 24; a longer `site` host, if the domain ever changes, can fail the build through the footer check, which then needs a shorter host or a new footer layout decision.
...
- `site` points at `jorgergo.dev`, which Go live confirmed (spec 0007); moving to another domain changes `site` and the `routes` host in `wrangler.jsonc` together (spec 0007 keeps them equal, and the www Redirect Rule follows in the dashboard), and every URL and footer follows.
...
- [x] Go live spec: confirm `jorgergo.dev` is free (or change `site`; the AC-17 footer check fails the build if the new host is too long), run the AC-14 share checks, and decide Cloudflare's AI crawler setting, since robots rules stay a zone setting. During the X check, confirm X shows the card's alt text; X may not read `og:image:alt`, and if it doesn't, a spec update adds `twitter:image:alt` (AC-5 and its page test change with it). Done in spec 0007: `jorgergo.dev` is the live host, the AC-14 share checks passed (`verify.md`), and all AI crawlers are allowed (0007 AC-11: AI bot blocking and the managed `robots.txt` are off, and the zone's `/robots.txt` is Cloudflare's comments only Content Signals text, which sets no rules). X reads `og:image:alt`: on 2026-09-26 the `alt` of X's card image matched both pages' `og:image:alt` exactly (its hover tooltip shows `og:title`, not the alt), so no `twitter:image:alt` is needed.
