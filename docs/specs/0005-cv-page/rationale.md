...
> ⚠️ Premise note: the scope's Release 2 promises a one page PDF that matches the CV page exactly, and the content on record (four roles, one volunteer role, a degree with seven courses, three awards, six certificates, five skill lines, twenty one technologies, three languages, two interest groups) runs to about eighty printed lines at 11pt, which is two pages. This spec makes the web page print cleanly across pages and leaves the one page question to the PDF spec, where it belongs: either the content is trimmed, or the PDF gets a denser layout, or the promise is dropped. Deciding it here would mean hiding content on paper that the page shows on screen.
...
**Cons**:
- Two new components, five helpers, and two small prop additions to spec 0003's shell (`printFooter`, `SiteFooter` `class`).
