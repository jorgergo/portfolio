...
- **Five small pure helpers in `cv-format.ts`**, all testable without Astro: `formatProfilePath`, `joinMeta`, `formatLanguage`, `groupConsecutive`, `spanOf`. Sorting happens before grouping, so the two Ford roles are adjacent (the current one sorts first, the closed one is the newest closed). A group links to the first role that has a `url` (the newest). Runner up: grouping inline in the page frontmatter, untestable.
...
B.S., Computer Science and Technology · GPA 4.0/4.0 (97/100)
                                                      Toluca, Mexico
...
CERTIFICATIONS
────────────────────────────────────────────────────────────
Catia 3DEXPERIENCE (Catia V6) for Beginners and Catia V5 Users
                                                             May 2026
Udemy
...
Mono everywhere except the `Prose` blocks (the summary paragraph, entry summaries, bullets, the coursework line), which are Plex Sans. Meta text (dates, locations, keys, the label line, the contact line) is `text-sm text-muted`. A long left text (a certificate name) wraps beside its right value at 480px and above; below that the right value drops under it.
...
|---|---|---|---|
| `CvEntry` (`src/components/CvEntry.astro`) | `title` (req) · `href?` · `meta?` (a date, fixed width) · `subtitle?` · `aside?` (a location, may wrap) · `summary?` · `highlights?: readonly string[]` · `as?: 'h3' \| 'h4'` (`h3`) · `splittable?: boolean` (`false`) · `class?` (onto the root) | the entry `div` of AC-4: two pair lines, an optional `Prose` body, the slot | n/a |
| `KeyedList` (`src/components/KeyedList.astro`) | `items: readonly { readonly key: string; readonly values: readonly string[] }[]` · `class?` (onto the `dl`) | the `dl` of AC-8; items with no values skipped | n/a |
...
| `formatProfilePath` (`src/lib/cv-format.ts`) | `(url: string) => string` | host without a leading `www.` plus the path without a trailing `/`; no scheme, port, query, or hash | none; input is a schema valid `https` URL |
...
The separator in `joinMeta` and in `KeyedList` is a space, a middle dot (U+00B7), and a space. `formatDateRange`, `formatMonth`, `formatLocation`, `sortNewestFirst`, and `sortByDateDesc` are spec 0002's helpers, unchanged.
...
| Header | contact link texts and hrefs | `basics.email` and `mailto:` plus it; `formatProfilePath(profile.url)` and `profile.url`, file order |
...
| Any section | whether it renders | the source array has at least one entry (Skills & interests: at least one keyed row with values) |
...
| Experience, Leadership | the company link | one role: its `url`; several: the first role with a `url`; none: plain text |
...
| Skills & interests | rows and their order | skills groups, technologies groups, `Languages`, interests groups with keywords, then one `Interests` row of the name only groups (AC-8) |
...
- Dates sit in the right column of line 1 and locations in the right column of line 2 in every section; a date span never shrinks, a location span may wrap.
...
- Helpers (Vitest): the outputs of AC-9 for `formatProfilePath`, `joinMeta`, `formatLanguage`, verifies **AC-9**.
...
1. Add `formatProfilePath`, `joinMeta`, `formatLanguage`, `groupConsecutive`, and `spanOf` to `src/lib/cv-format.ts` with their Vitest cases in `src/lib/cv-format.test.ts` tagged `spec 0005 AC-9` (no Astro needed), satisfies **AC-9**.
...
3. Add the full width light and dark blocks with `CvEntry` (single, and a `splittable` group with two `h4` roles) and `KeyedList` to `/styleguide`, add their anatomy checks to `e2e/styleguide.spec.ts`, and update `design.md` (eleven components, the two new rules, the hairline heading rule, the pair rule, the weight line, the spacing meanings, the bullet style, the print rules), satisfies **AC-13**.
4. Add the `cv page` block to `e2e/site.spec.ts` (header, grouping, 479px and 480px, print, no script and three fonts, Tab order from the fixture), run the gate (`pnpm build`, `pnpm lint`, `pnpm format:check`, `pnpm test`, `pnpm exec playwright test`) and the manual steps in [verify.md](verify.md), satisfies **AC-10**, **AC-11**, **AC-12**, **AC-14**.
...
- Zero JavaScript, no new dependency, no schema change; the two components and five helpers are small, pure, and covered by tests, and the Release 2 PDF can build on the same helpers.
...
**Negative / tradeoffs**:
- The content on record prints to about two pages at 11pt; the Release 2 PDF's one page promise needs trimming or a denser layout, decided there, not here.
...
- [ ] CV PDF download spec (Release 2): build from the same helpers and `CvEntry` rules; decide how a one page PDF is reached (content trimming, a compact print layout, or dropping the one page promise) since the web page prints to two.
...
- [ ] `/sync` after the build: add `CvEntry`, `KeyedList`, the `printFooter` prop, and the `SiteFooter` `class` prop to `AGENTS.md`'s component notes and to `design.md`'s count; add `docs/.agent-cache/` to `.gitignore` or commit the research cache on purpose (it is untracked today).
